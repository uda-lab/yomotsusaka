...
| `wait_timeout`                          | `operational_smoke` runpod phase / `scripts/manage_runpod.py` (mirrored) | Pod created but did not become healthy within the configured wait budget; library performs best-effort cleanup (stop_pod) before re-raising; Pod was successfully deleted.      | Report `wait_timeout`. Owner may inspect RunPod console; no further agent action required (Pod was deleted).                                                  |
| `wait_timeout_cleanup_failed`           | `operational_smoke` runpod phase / `scripts/manage_runpod.py` (mirrored) | Pod created but did not become healthy (wait_timeout) AND the subsequent best-effort cleanup also failed; Pod may still be running and billing. Routes to `failed_owner_action`.| Report `wait_timeout_cleanup_failed`. Owner must manually delete the Pod via RunPod console or `runpodctl`; do not retry without owner review.                |
