...
| `src/yomotsusaka/execution_gateway.py` | functional stub | Declares the Chikaeshi spec surface — `ExecutionScope` (`PRIVATE_BOUNDARY`, `ORDINARY_AGENT`; distinct from `ResolverScope`), `ExecutionRequest`, `ExecutionResponse`, `ExecutionFailureReason`, plus the `ExecutionFailure` exception (reserved for programmer-error paths; the dispatcher never raises it). `ExecutionResponse` carries an opaque audit-record id, status, public-handle artifacts, scrubbed stdout/stderr fragments, plus optional `reason: ExecutionFailureReason` and scrubbed `detail` for failure paths. The legacy `ExecutionGateway.execute(handle, op, params)` method still returns the `{"status": "stub", ...}` dict for callers that pre-date #43. The real dispatch lives on `boundary.execute_request(request, *, tenant\|vault_root)`: it enforces the closed-set `TEMPLATES` registry from `src/yomotsusaka/templates.py`, the scope gate, and scrubs stdout/stderr via `src/yomotsusaka/scrubber.py` before returning. Every call writes one append-only JSONL row to `<vault_root>/audit/restoration.jsonl` via `src/yomotsusaka/audit.py` (including denials and schema-invalid requests); the `policy_profile` / `approval_ticket` columns are reserved-as-`None` for #44. When the required audit write itself fails (`AuditError` or `OSError`), the call returns `ExecutionResponse(status="failed", reason=AuditWriteFailed)` — no path returns `status="accepted"` after a failed success-audit write, and no failure path echoes the original classification as if the audit had landed (see `docs/chikaeshi.md` §4.1 and issue #59). Failures are RETURNED as `ExecutionResponse(status="failed", reason=...)`; never raised. See `docs/chikaeshi.md` for the §13 / `docs/backend-promotion.md` §4 specification. | Private execution gateway boundary (architecture §13) — template-based dispatch with scrubbed I/O. Real container runtime / Linux capability enforcement remains deferred to a separate post-MVP-3 child issue. |
